...
def plot_spikes_and_voltage(
    ...
    axs[1].plot(time_arr, y_soma_gt, label="Soma GT", color='red')
    axs[1].plot(time_arr, y_soma_pred, label="Soma Pred", color='blue', alpha=0.7)
    axs[1].set_ylabel("Soma Voltage (mV)")
    axs[1].set_xlabel("Time step")
    axs[1].legend()
    axs[1].set_title("Soma Voltage Ground Truth vs Prediction")
...
if __name__ == "__main__":
    ...
    valid_idx = np.random.choice(len(valid_data_full), size=4000, replace=False)
    ...
    criterion_spikes = nn.BCEWithLogitsLoss(pos_weight=pos_weight)
    ...
    for epoch in range(start_epoch, end_epoch):
        ...
        for batch_idx, batch in enumerate(progress_bar):
            X_spikes   = batch['sps_in'].float().to(device)
            y_soma_GT  = batch['somatic_voltage_out'].float().squeeze().to(device)
            y_soma_GT  = torch.clamp(y_soma_GT, max=-40.0)            # enforce ceiling
            y_soma_GT  = (y_soma_GT - V_bias) / V_scale               # normalize
            ...
            taylor_soma  = temporal_derivative_mse(y_soma_pred, y_soma_GT)
            ...
            if (iter_num + 1) % num_batches_per_valid_eval == 0:
                ...
                with torch.no_grad():
                    for vb in valid_dataloader:
                        ...
                        yv_soma = vb['somatic_voltage_out'].float().squeeze().to(device)
                        yv_soma = torch.clamp(yv_soma, max=-40.0)
                        yv_soma = (yv_soma - V_bias) / V_scale
                        ...
                        lv_soma = criterion_soma(pv_soma, yv_soma)
    ...
    save_dir_learning_curves = '/ems/elsc-labs/london-m/lior.avraham1/layer5/training_net_job_output3/learning_curves'
    ...
    def predict_on_all_simulations(model, dataset, batch_size=batch_size, V_bias=V_bias, V_scale=V_scale):
        """Run predictions on entire dataset and optionally save trace plots."""
        model.eval()
        ...
        with torch.no_grad():
            for i, b in enumerate(dataloader):
                ...
                y_sv  = torch.clamp(y_sv, max=-40.0)
                print("max GT after clamp (batch %d): %.3f" % (i, float(y_sv.max().cpu())))
                assert float(y_sv.max().detach().cpu()) <= -39.999, "GT soma not clamped!"

                y_spgt = b['sps_out'].float().squeeze().to(dev)
                y_sp_pred, y_soma_pred = model(X_sp)

                # rescale voltage
                y_soma_pred = V_scale * y_soma_pred + V_bias

                print("GT clamp check: max y_soma_gt =", float(y_sv.max().cpu()))
                print("Pred range (mV): min =", float(y_soma_pred.min().cpu()),
                      "max =", float(y_soma_pred.max().cpu()))

                # to numpy
                spikes_gt_np = y_spgt.cpu().numpy()
                spikes_pred_np = torch.sigmoid(y_sp_pred).cpu().numpy()
                soma_gt_np = y_sv.cpu().numpy()
                soma_pred_np = y_soma_pred.cpu().numpy()

                all_y_spikes_pred.append(spikes_pred_np)
                all_y_soma_pred.append(soma_pred_np)
                all_y_spikes_gt.append(spikes_gt_np)
                all_y_soma_gt.append(soma_gt_np)

        # concat
        y_spikes_pred = np.concatenate([x.squeeze() for x in all_y_spikes_pred], axis=0)
        ...
        return y_spikes_pred, y_soma_pred, y_spikes_gt, y_soma_gt

    def calculate_metrics(y_spikes_gt, y_spikes_pred, y_soma_gt, y_soma_pred,
                          criterion_cls, criterion_reg, V_bias=V_bias, V_scale=V_scale,
                          desired_false_positive_rate=0.01, num_datapoints_in_scatter=20000,
                          print_metrics=True):
        """Calculate performance metrics (Option A: soma voltages in mV)."""
        ...
        valid_idxs = np.where(fpr <= desired_false_positive_rate)[0]
        desired_fp_index = valid_idxs[-1] if len(valid_idxs) else 0
        thresh = thresholds[desired_fp_index]
        ...
        thresh = thresholds[desired_fp_index]
        print("desired false positive rate = 0.01")
        print(f'optimal threshold: {thresh}')
        y_spikes_pred_binary = (y_spikes_pred > thresh).astype(int)
        ...
        # Loss values
        spikes_loss = criterion_cls(torch.tensor(y_spikes_pred, device=device),
                                    torch.tensor(y_spikes_gt.astype(float), device=device))
        soma_loss = criterion_reg(torch.tensor(y_soma_pred, device=device),
                                  torch.tensor(y_soma_gt, device=device))

        # Scatter plot subsample
        selected_indices = np.random.choice(len(y_soma_gt.ravel()), num_datapoints_in_scatter, replace=True)
    ...
    def plot_evaluation_figures(metrics_dict, voltage_granularity=8, voltage_setpoint=V_bias):
        ...
        save_dir = '/ems/elsc-labs/london-m/lior.avraham1/layer5/training_net_job_output3/evaluation_figures'
        os.makedirs(save_dir, exist_ok=True)
        matplotlib.use('Agg')
    ...
    def evaluate_model_on_dataset(model, dataset, criterion_cls, criterion_reg,
        ...
        # predictions
        y_spikes_pred, y_soma_pred, y_spikes_gt, y_soma_gt = predict_on_all_simulations(
        ...
        return metrics_dict, y_spikes_pred, y_soma_pred, y_spikes_gt, y_soma_gt
    
    # ============================== Load best checkpoint for final eval ==============================
    best_auc_path  = os.path.join(models_folder, f"{model.short_name}_best_auc.pt")
    best_loss_path = os.path.join(models_folder, f"{model.short_name}_best.pt")

    if os.path.isfile(best_auc_path):
        print(f"[INFO] Loading best-by-AUC weights: {best_auc_path}")
        model.load_state_dict(torch.load(best_auc_path, map_location=device))
    elif os.path.isfile(best_loss_path):
        print(f"[INFO] Loading best-by-loss weights: {best_loss_path}")
        model.load_state_dict(torch.load(best_loss_path, map_location=device))
    else:
        print("[WARN] No best checkpoint found; evaluating last weights in memory.")

    # Evaluate model on validation set
    valid_metrics_dict, y_spikes_pred, y_soma_pred, y_spikes_gt, y_soma_gt = evaluate_model_on_dataset(
    ...
    train_metrics_dict, y_spikes_pred_train, y_soma_pred_train, y_spikes_gt_train, y_soma_gt_train = evaluate_model_on_dataset(
    ...
    save_trace_dir = '/ems/elsc-labs/london-m/lior.avraham1/layer5/training_net_job_output3/trace_graphs'
    os.makedirs(save_trace_dir, exist_ok=True)

    n_samples = y_spikes_gt.shape[0] if y_spikes_gt.ndim > 1 else 1
    for i in range(n_samples):
        assert np.max(y_soma_gt[i]) <= -40.0, f"Unclamped GT value found! max={np.max(y_soma_gt[i])}"
        plot_spikes_and_voltage(
            y_spikes_gt=y_spikes_gt[i],
            y_spikes_pred=y_spikes_pred[i],
    ...
    for key in valid_metrics_dict.keys():
        print(f"  '{key}'")
    print('--------------------------------------------------')
    interesting_keys = ['requested_false_positive_rate', 'true_positive_at_FP', 'AUC_score',
                        'soma_explained_variance_percent', 'soma_RMSE', 'soma_MAE',
